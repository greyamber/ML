...
class Configure(object):
    def __init__(self, action_len=2, obs=[4]):
        ...
        # off-policy
        # max_buffer = update_delay = batch_size means on-policy
...
class ACAgent(object):
    def __init__(self, cfg=Configure()):
        ...
        # s, a, r, v, s_(如果critic更新速度快于buffer更新速度，v是误差比较大的项)
        # 不过dqn使用该策略问题不大，这里进行尝试
        self.buffer = [[],[],[],[],[]]
        self.pointer = 0
    ...
    def train(self, s, a, r, v, s_, done, update):
        # 使用v(s_,theta)来估计Q
        # 结果：根本收敛不了- -？
        ...
        # Q用r+gama*v(s_|theta)估计
        v_estimate = self.sess.run(self.critic_out, feed_dict={self.input_state: self.buffer[4]})
        Q = rewards + self.cfg.gama * np.reshape(v_estimate, [-1])
        self.buffer = [[],[],[],[],[]]
        # A用Q - V估计
        advantages = Q - value
        feed_dict = {self.input_state: states, self.actor_labels: labels,
                     ...
                     self.vf_next: v_next}
        _, loss, closs = self.sess.run([self.train_op, self.loss, self.critic_loss],
                                       feed_dict=feed_dict)
        return closs
    ...
    for i in range(train_iter):
        ...
        for _ in range(env._max_episode_steps):
            ...
            if k % agent.cfg.update_delay == 0:
                closs = agent.train(s, a, r, v, s_, done, True)
                k = 1
            else:
                closs = agent.train(s, a, r, v, s_, done, False)
